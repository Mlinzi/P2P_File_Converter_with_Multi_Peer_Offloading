...
def get_available_outputs(input_format: str) -> list[str]:
    ...
    for t in targets:
        if fmt in AUDIO_FORMATS or fmt in VIDEO_FORMATS:
            if FFMPEG: result.append(t)
        elif fmt in ALL_DOCUMENT_FORMATS:
            if t == 'pdf':
                can = SOFFICE
                can = can or (HAS_DOCX2PDF and fmt in WORD_FORMATS)
                can = can or (HAS_COMTYPES  and fmt in {'pptx', 'ppt'})
                can = can or fmt in {'pptx', 'ppt'}                    # python-pptx fallback
                if can: result.append(t)
            else:
                if SOFFICE: result.append(t)
        else:
...
def _convert_document(input_path: Path, output_format: str, output_dir: Path, output_path: Path) -> Path:
    ...
    PDF output priority:
      ...
      pptx/ppt      → LibreOffice           → PowerPoint COM  → python-pptx+reportlab
...
def _convert_pptx_python(input_path: Path, output_path: Path) -> Path:
    ...
    # Slide dimensions in EMUs → inches
    ...
    page_size = (slide_w * 72, slide_h * 72)   # points (72 pt = 1 inch)
    ...
    for slide in prs.slides:
        ...
        img_path_tmp = output_path.with_suffix('.tmp_slide.png')
        img_obj.save(str(img_path_tmp))

        c.drawImage(str(img_path_tmp), 0, 0, width=page_size[0], height=page_size[1])
        c.showPage()
