...
class Discovery:
    ...
    def _udp_listen(self):
        while self._udp_sock:
            try:
                ...
                # Reply so the other peer learns about us
                try:
    ...
    def _udp_scan(self):
        ...
        # Determine scan width: private /16 or just local /24
        a = int(parts[0]); b = int(parts[1])
        if a == 10:
            third_range = range(256)                        # 10.x.0-255.y
        elif a == 172 and 16 <= b <= 31:
            third_range = range(16, 32)                     # 172.16-31.0-255.y
        elif a == 192 and b == 168:
            third_range = range(256)                        # 192.168.0-255.y
        else:
            third_range = [int(parts[2])]                   # public IP: /24 only
        ...
        # Re-announce on local subnet every 30 s (lightweight keepalive)
        prefix = '.'.join(parts[:3])
